...
bool isValidWindowsFileName(String textt) {
  ...
  String baseName = textt;
  if (lastDotIndex != -1) {
    baseName = textt.substring(0, lastDotIndex); // 最後のドットまでの部分を取得
  }
...
int nowposit(){
  return (positpoint + imano_page * positpointmax + imano_page) ;
}
...
void listSDRootContents(int pagetax,String Directtory) {
  ...
  if (pagetax < 0 || pagetax >= maxpage) {
    M5.Lcd.clear();
    M5.Lcd.setCursor(0, 0);
    M5.Lcd.printf("Page %d is out of range (0-%d)", pagetax, maxpage - 1);
    delay(2000); // メッセージ表示のため少し待つ
    return; // 無効なページの場合はここで終了
  }
